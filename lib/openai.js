 ...
 *   barcode_or_line: "string",  // linha digitável / código de barras (se aparecer)
 *   pix_key: "string",          // chave/identificador do PIX (se aparecer)
 *   notes: "string",
...
`.trim();
  ...
  // fallback seguro se o modelo devolver algo fora do JSON
  ...
  return {
    method: toStr(parsed.method) || "unknown",
    amount: toStr(parsed.amount),
    date: toStr(parsed.date),
    payer_doc: toStr(parsed.payer_doc),
    beneficiary: toStr(parsed.beneficiary),
    barcode_present: Boolean(parsed.barcode_present),
    barcode_or_line: toStr(parsed.barcode_or_line),
    pix_key: toStr(parsed.pix_key),
    notes: toStr(parsed.notes),
    summaryText: toStr(parsed.summaryText),
  };
}
