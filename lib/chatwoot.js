// lib/chatwoot.js
// (arquivo completo)
// ✅ Mantém implementação simples e estável.
// ✅ Retorna sempre { ok, status, body } nos métodos.
// ✅ downloadAttachmentAsDataUrl devolve dataUri/base64 para enviar ao OpenAI.
...
export async function chatwootSignInIfNeeded({ baseUrl, email, password, force = false }) {
  ...
  // (Se quiser cache depois, dá para adicionar sem mexer no resto do repo.)
...
export async function removeLabel({ baseUrl, accountId, conversationId, headers, label }) {
  const res = await fetch(
    `${baseUrl}/api/v1/accounts/${accountId}/conversations/${conversationId}/labels/${label}`,
...
export async function downloadAttachmentAsDataUrl({ baseUrl, headers, dataUrl }) {
  // dataUrl normalmente é uma URL do próprio Chatwoot, e precisa dos headers de auth
