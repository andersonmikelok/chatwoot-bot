// lib/receitanet.js
// BUILD_ID: fix4-2026-02-19T23:30-0300
...
async function rnPostFormWithQuery({ baseUrl, path, queryParams, form }) {
  ...
  const t = setTimeout(() => ctrl.abort(), RN_TIMEOUT_MS);
  let res;
  try {
...
export async function rnNotificacaoPagamento({ baseUrl, token, app, idCliente, contato }) {
  ...
  if (!resp.ok) return { ok: false, status: resp.status, data: resp.json, text: resp.text, url: resp.url };
  return { ok: true, status: resp.status, data: resp.json };
}
