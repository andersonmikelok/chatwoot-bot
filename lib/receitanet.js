 ...
 * - NÃO derruba o bot em erro 500/502/etc
 * - Em caso de falha, retorna found:false com status/body
 */
export async function rnFindClient({ baseUrl, token, app, cpfcnpj, phone, idCliente }) {
  ...
  if (!resp.ok) {
    return {
      ...
      url: resp.url,
      error: `ReceitaNet /clientes falhou (${resp.status})`,
    };
  }
...
/**
 * ✅ Alteração importante:
 * - Em erro 500/502/etc, NÃO derruba o bot: retorna []
 */
...
/**
 * ✅ Alteração importante:
 * - Em erro 500/502/etc, NÃO derruba o bot: retorna ok:false
 */
export async function rnVerificarAcesso({ baseUrl, token, app, idCliente, contato }) {
  ...
  if (!resp.ok) {
    return { ok: false, status: resp.status, body: resp.json || resp.text, url: resp.url };
  }
...
function mapBoletoFields(raw) {
  ...
  const vencimento = r.vencimento || r.data_vencimento || r.dt_vencimento || r.vcto || r.due_date || "";
  ...
  const statusRaw =
    r.status || r.situacao || r.sit || r.estado || r.state || r.pagamento_status || r.status_pagamento || "";

  const pagamento = r.pagamento || r.data_pagamento || r.dt_pagamento || r.paid_at || r.paid_date || "";
...
function isPaidOrClosed(b) {
  ...
  if (st.includes("pag")) return true;
 ...
 * - Se existir pendente vencido: pega o mais antigo
